...
/////////////////////////////////////////////////
reg [3:0] S, S_next;
localparam S_IDLE = 0, S_LOAD_WEIGHT = 1,S_LOAD_BIAS = 2, S_FMA = 3, S_FMA_RESULT = 4, S_ADD =5, S_RELU = 6, 
                    S_COMP = 7, S_RESULT = 8, S_LOAD_I_IMG = 9;
...
begin
    ...
    case(S)
        S_IDLE:begin
            ...
            else if(en_i && we_i && addr_i == 32'hC410_0008) begin
                S_next = S_LOAD_BIAS;
            end
        ...
        S_LOAD_BIAS:begin
            if(load_bias_cnt == total_bias)begin
                S_next = S_IDLE;
            end
        end
        S_LOAD_I_IMG:begin
...
//load bias data
/////////////////////////////////////////////////
reg [5:0] load_bias_cnt;
reg [5:0] total_bias;
(* ram_style="block" *) reg [XLEN-1:0] bias_data[9:0];
...
always @(posedge clk_i) begin
    if(rst_i)begin
        ...
        if(en_i && we_i && addr_i == 32'hC410_000c)begin
            bias_data[load_bias_cnt] <= data_i;
            load_bias_cnt <= load_bias_cnt + 1;
        end
    end
end
...
reg [XLEN-1:0] add_dataA;
reg [XLEN-1:0] add_dataB;
wire [XLEN-1:0] add_result_data;
wire add_result_valid;
reg add_data_valid;
reg [XLEN-1:0] add_result_reg;

wire [XLEN-1:0] cmp_data = (comp_result_data) ? 0 : add_result_reg;
...
begin
    if(rst_i)begin
        ...
        add_dataA <= result_reg;
        add_dataB <= bias_data[curr_idx];
        add_data_valid <= 1;
    end
    else if(S == S_RELU)begin
        if(add_result_valid)begin
            comp_dataA <= add_result_data;
            comp_dataB <= 0;
            comp_data_valid <= 1;
...
assign ready_o = S == S_RESULT || S == S_IDLE || S == S_LOAD_BIAS || S == S_LOAD_WEIGHT || S == S_LOAD_I_IMG;
...
FP_ADD fp_add(
    ...
    .s_axis_a_tdata(add_dataA),
    .s_axis_b_tvalid(add_data_valid),
    .s_axis_b_tdata(add_dataB),
...
// profiler
(*mark_debug = "true"*) reg [31 :0] load_weight_time;
(*mark_debug = "true"*) reg [31 :0] load_image_time;
(*mark_debug = "true"*) reg [31 :0] send_out_image_cnt;
(*mark_debug = "true"*) reg [31 :0] fma_cnt;
(*mark_debug = "true"*) reg [31 :0] add_cnt;
(*mark_debug = "true"*) reg [31 :0] relu_cnt;
(*mark_debug = "true"*) reg [31 :0] comp_cnt;
...
begin
    if(rst_i)begin
        ...
        load_weight_time <= load_weight_time + 1;
    end
    else if(S == S_LOAD_I_IMG)begin
        load_image_time <= load_image_time + 1;
    end
    else if(S == S_RESULT)begin
        send_out_image_cnt <= send_out_image_cnt + 1;
    end
    else if(S == S_FMA)begin
        fma_cnt <= fma_cnt + 1;
    end
